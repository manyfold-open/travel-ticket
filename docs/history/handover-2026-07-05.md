...
> Historical snapshot, not a current task list. Start from
> `docs/README.md` and `README.md`; use this file only for implementation history
> and the recorded pitfalls.
>
> 原開場順序：`docs/design-system.md` → `docs/history/feature-specs-2026-07-03.md` → 本檔。
...
- **Zack 真機驗過**：印刷版 Cmd+P、Start Day 1 導航、docs/product.md 內容——都沒問題。
...
- **icons**：純 Node `zlib` 手刻 PNG 編碼器＋光柵化（3× supersample），畫車票圖標
  ...
  mirror docs/design-system.md tokens（--night/--paper/--rail/--ink/--rail-deep），無新 hex。
...
- **改前端先讀 docs/design-system.md**；新色先進「衍生 tokens」表再 var() 引用，禁裸 hex。
- Skills 編隊（docs/history/feature-specs-2026-07-03.md 有表）：動效正名 emil `animation-vocabulary`、實作查 gsap 官方
